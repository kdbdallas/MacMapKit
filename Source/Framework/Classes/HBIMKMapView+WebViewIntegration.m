...
@protocol HBIMKDraggableAnnotation <NSObject>
...
@implementation HBIMKMapView (WebViewIntegration)
...
- (void)updateOverlayZIndexes
{
    ...
    WebScriptObject *webScriptObject = [webView windowScriptObject];
    for (id <HBIMKOverlay> overlay in overlays)
    {
...
- (void)updateAnnotationZIndexes
{
    ...
    NSArray *sortedAnnotations = [annotations sortedArrayUsingComparator: ^(id <HBIMKAnnotation> ann1, id <HBIMKAnnotation> ann2) {
    ...
    for (id <HBIMKAnnotation> annotation in sortedAnnotations)
    {
...
- (void)annotationScriptObjectSelected:(WebScriptObject *)annotationScriptObject
{
    ...
    for (id <HBIMKAnnotation> annotation in annotations)
    {
...
- (void)annotationScriptObjectDragStart:(WebScriptObject *)annotationScriptObject
{
    ...
    for (id <HBIMKAnnotation> annotation in annotations)
    {
        ...
        if ([scriptObject isEqual:annotationScriptObject])
        {
            ...
            {
                HBIMKAnnotationView *view = (HBIMKAnnotationView *)[annotationViews objectForKey: annotation];
                view.dragState = HBIMKAnnotationViewDragStateStarting;
                [self delegateAnnotationView:view didChangeDragState:HBIMKAnnotationViewDragStateStarting fromOldState:HBIMKAnnotationViewDragStateNone];
            }
        }
    }
}
    ...
    for (id <HBIMKAnnotation> annotation in annotations)
    {
        ...
        if ([scriptObject isEqual:annotationScriptObject])
        {
            ...
            if ([annotation respondsToSelector:@selector(setCoordinate:)])
            {
                ...
                [(id <HBIMKDraggableAnnotation> )annotation setCoordinate:newCoordinate];
                HBIMKAnnotationView *view = (HBIMKAnnotationView *)[annotationViews objectForKey: annotation];
                if (view.dragState != HBIMKAnnotationViewDragStateDragging)
                {
                    view.dragState = HBIMKAnnotationViewDragStateNone;
                    [self delegateAnnotationView:view didChangeDragState:HBIMKAnnotationViewDragStateDragging fromOldState:HBIMKAnnotationViewDragStateStarting];
                }
            }
        }
    }
    ...
    for (id <HBIMKAnnotation> annotation in annotations)
    {
        ...
        if ([scriptObject isEqual:annotationScriptObject])
        {
            ...
            if ([annotation respondsToSelector:@selector(setCoordinate:)])
            {
                ...
                HBIMKAnnotationView *view = (HBIMKAnnotationView *)[annotationViews objectForKey: annotation];
                view.dragState = HBIMKAnnotationViewDragStateNone;
                [self delegateAnnotationView:view didChangeDragState:HBIMKAnnotationViewDragStateNone fromOldState:HBIMKAnnotationViewDragStateDragging];
            }
        }
    }
}
...
- (void)webviewReportingClick:(NSString *)jsonEncodedLatLng
{
    ...
    for (id <HBIMKAnnotation> annotation in currentlySelectedAnnotations)
    {
...
- (void)annotationScriptObjectRightClick:(WebScriptObject *)annotationScriptObject
{
    ...
    for (id <HBIMKAnnotation> annotation in annotations)
    {
        ...
        if ([scriptObject isEqual:annotationScriptObject])
        {
	    annotationView = (HBIMKAnnotationView *)[annotationViews objectForKey: annotation];
        }
    }
